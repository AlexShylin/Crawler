...
public class CrawlersServiceV1Test {

    @InjectMocks
    private CrawlersServiceV1 crawlersService = new CrawlersServiceV1();
    ...
    public void testLaunch() {
        ...
        doNothing().when(requestValidator).validateLaunchParams(url, delay);
        ...
        Long returnedId = crawlersService.launch(url, delay).getId();

        // validate order call
        InOrder inOrder = inOrder(crawlerFactory, requestValidator, processingCrawlers, crawler);
        inOrder.verify(requestValidator).validateLaunchParams(url, delay);
